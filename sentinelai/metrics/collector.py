...
class MetricsCollector:
    ...
    def export_prometheus(self) -> str:
        ...
        for name, series in self._series.items():
            if series.count > 0:
                lines.append(f"sentinelai_{name}_count {series.count}")
                lines.append(f"sentinelai_{name}_mean {series.mean()}")
                lines.append(f"sentinelai_{name}_p50 {series.percentile(50)}")
                lines.append(f"sentinelai_{name}_p90 {series.percentile(90)}")
                lines.append(f"sentinelai_{name}_p99 {series.percentile(99)}")
